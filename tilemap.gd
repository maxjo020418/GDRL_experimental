...
func _ready() -> void:
	...
	# connecting all the needed signals
	for agent in agents:
...
func get_reward(coords: Vector2i) -> float:
	if coords in tile_data:
		return tile_data[coords].get_custom_data('reward')
	else:
		push_error(str(coords), ' does not exist in the map!')
		return 0
